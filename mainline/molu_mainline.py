# -*- coding: utf-8 -*-
"""Molu_chatbot_0919.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1uIvmfyXP3rNtfwjKQVIBqzhheLyWgV6u
...
class DataConfig:
    ...
    shuffle: bool = True
    seed: int = 42
    fallback_fields: Sequence[str] = field(default_factory=lambda: TEXT_FALLBACK_KEYS)
...
def _select_text(utterance: dict, cfg: DataConfig) -> str:
    if cfg.text_field not in {"form", "original_form"}:
...
def load_dialogue_pairs(cfg: DataConfig) -> Tuple[List[Tuple[str, str]], List[str], dict]:
    path = cfg.resolve_path()
    pairs: List[Tuple[str, str]] = []
    mono_docs: List[str] = []
    missing_speaker_docs = 0
    n_files = 0
    saw_conversation_doc = False
    ...
    with zipfile.ZipFile(path) as zf:
        for name in zf.namelist():
            ...
            if docs:
                saw_conversation_doc = True
                for doc in docs:
                    utts = doc.get("utterance") or []
                    if not isinstance(utts, list):
                        continue
                    utts = sorted(
                        (utt for utt in utts if isinstance(utt, dict)),
                        key=lambda u: (
                    ...
                    for utt in utts:
                        ...
                        if len(text) > cfg.max_text_chars:
                            continue
                        if cfg.require_korean and not is_korean(text):
                            continue
                    ...
                    if doc_text and cfg.min_text_chars <= len(doc_text) <= cfg.max_text_chars:
                        if not cfg.require_korean or is_korean(doc_text):
                    ...
                    if any(speaker is None for _, speaker in cleaned_turns):
                        ...
                        logger.warning(
                            "Skipping document %s due to missing speaker_id",
                            doc.get("id"),
                        )
                        continue
                    ...
                    def flush_buffer(speaker: str | None) -> None:
                        ...
                        if not combined:
                            return
                        if not (
                            cfg.min_text_chars <= len(combined) <= cfg.max_text_chars
                        ):
                            return
                        if cfg.require_korean and not is_korean(combined):
                            return
                        if speaker is not None:
                    ...
                    if cfg.pairing_mode == "turn_change":
                        for (a, speaker_a), (b, speaker_b) in zip(
                            sequence, sequence[1:]
                        ):
                        ...
                        pairs.extend(
                            (a, b) for (a, _), (b, _) in zip(sequence, sequence[1:])
                        )
                    else:
                        raise ValueError(
                            f"Unsupported pairing_mode: {cfg.pairing_mode}"
                        )
            else:
                ...
                for text in prose_texts:
                    if not text:
                        continue
                    if not (
                        cfg.min_text_chars <= len(text) <= cfg.max_text_chars
                    ):
                        continue
                    if cfg.require_korean and not is_korean(text):
                        continue
    ...
    if saw_conversation_doc and not pairs:
        raise RuntimeError("No dialogue pairs could be constructed from the dataset.")

    stats = {"n_files": n_files, "missing_speaker_docs": missing_speaker_docs}
    return pairs, mono_docs, stats
...
RUN_MAIN = os.environ.get("MOLU_SKIP_EXECUTION") != "1"
...
if RUN_MAIN:
    ...
    pairs, mono_docs, stats = load_dialogue_pairs(cfg)
    n_files = stats.get("n_files", 0)
    missing_speaker_docs = stats.get("missing_speaker_docs", 0)
